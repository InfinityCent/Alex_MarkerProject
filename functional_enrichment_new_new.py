...
out_dir = 'C:\\Users\\peree\\OneDrive\\Desktop\\CompBio_Code\\fun_enrich_files\\combined_cluster_outputs'

split_CC = ['no_cellcycle', 'with_cellcycle']
split_AS = ['with_AreaShape', 'without_AreaShape']
split_genes = ['all_genes', 'ess_only', 'no_vesicle', 'noness_only']
...
for CC in split_CC:
    for AS in split_AS:
        for gene_dir in split_genes:
            ...
            ds_dir = os.path.join(cluster_dir, CC, AS, gene_dir)
            all_files = os.listdir(ds_dir)

            for i in range(1, len(all_files) + 1, 2):
                query_file = all_files[i]

                query_path = os.path.join(ds_dir, query_file)
                ...
                print(f'{CC}/{AS}/{gene_dir}/{query_file}')
                #print(f'{CC}/{AS}/{gene_dir} ‒ {strain_df.shape} strain_df: {strain_df.shape[0]} ‒ {len_background} unique ORFs ‒ N: {N}')
                ...
                for ontology in go_slim_ontologies:
                    ...
                    file_name = query_file.split("Cluster")[1]
                    file_name = file_name.split("_")
                    file_name = "Cluster" + file_name[1] + file_name[2] + file_name[3]

                    out_path = os.path.join(out_dir, CC, AS, gene_dir, '%s-%s.xlsx' % (file_name, ontology_name))
