...
for marker in markers:
    for rep in reps:
        for dataset in datasets:
            ds_dir = '%s_%s_PCA%s' % (marker, rep, dataset)  # marker_rep_PCAdataset
            strain_path = os.path.join(cluster_dir, '%s\\Strain_profile.csv' % ds_dir)
            strain_df = p.read_csv(strain_path)

            # NEEDS TO BE UPDATED
            #background = set(strain_df['ORF'].unique().tolist())
            ...
            print(f'{rep} ‒ {strain_df.shape} strain_df: {strain_df.shape[0]} ‒ {len_background} unique ORFs ‒ N: {N}')

            # get cluster column names for all cell cycle stages
            stages = ['G1 phase', 'MA phase', 'MA-single', 'S/G2 phase', 'T phase']
            ...
            for ontology in go_slim_ontologies:
                ...
                # NEEDS TO BE UPDATED
                # '/home/morphology/shared/morphology/Users/Alex/MarkerProject/functional_enrichment_analysis/clustering_profiles/Cdc11_all_PCA_combinedMA/enrichment_analysis/Cdc11_all_mc_slim.xlsx'
                out_path = os.path.join(cluster_dir, '%s/enrichment_analysis/%s_%s_%s.xlsx' % (ds_dir, marker, rep, ontology_name))
                ...
                # go through each cell cycle stage + appended cluster and sort from greatest to least
                for stage in stages:
                    ...
                    query_path = 'C:\\Users\\peree\\OneDrive\\Desktop\\CompBio_Code\\fun_enrich_files\\Combined_cluster_profiles_penetrant_strains_combinedMA_geneclusters_h0.75_all_genes_FE_QUERY_GENES.csv'
                    query2 = p.read_csv(query_path)
                    query2 = set(query2['ORF'])
...
### QC CHECKS
# for annotation, terms in annotation_dict.items():
#     print("%s — number of terms: %d" % (annotation, len(terms)))
#     for term in terms:
#         print('\t %s' % term)
