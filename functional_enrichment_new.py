...
# Global variables
cluster_dir = 'C:\\Users\\peree\\OneDrive\\Desktop\\CompBio_Code\\fun_enrich_files\\per_marker'
out_dir = 'C:\\Users\\peree\\OneDrive\\Desktop\\CompBio_Code\\fun_enrich_files\\per_marker_outputs'
...
for AS in split_AS:
    ...
    ds_dir = os.path.join(cluster_dir, AS)
    all_files = os.listdir(ds_dir)
    for marker in all_files:
        new_ds_dir = os.path.join(ds_dir, marker)
        marker_files = os.listdir(new_ds_dir)

        for i in range(1, len(marker_files) + 1, 2):
            query_file = marker_files[i]
            query_path = os.path.join(new_ds_dir, query_file)
            ...
            print(f'{AS}/{marker}/{query_file}')
            #print(f'{AS}/{f}/{query_file} ‒ {strain_df.shape} strain_df: {strain_df.shape[0]} ‒ {len_background} unique ORFs ‒ N: {N}')
            # get cluster column names for all cell cycle stages
            #stages = []
            #columns = strain_df.columns
            #for c in columns:
            #    if c not in ['ORF', 'Name', 'Allele', 'Strain ID',
            #                 'Num_cells', 'Num_cells_outliers',
            #                 'Penetrance', 'normal']:
            #        stages.append(c)
            ...
            for ontology in go_slim_ontologies:
                ...
                # ClusterXofY-go_slim_c/p
                file_name = query_file.split("_")
                file_name = file_name[2] + file_name[3] + file_name[4]
                file_name = file_name.split("-")
                file_name = file_name[0] + file_name[1]

                out_path = os.path.join(out_dir, AS, marker, '%s-%s.xlsx' % (file_name, ontology_name))
                writer = p.ExcelWriter(out_path, engine='xlsxwriter')
                print(os.path.relpath(out_path, start=cluster_dir))  # get relative output path from cluster_di
                # go through each appended cluster, make a copy of strain_df, and sort from greatest to least

                sheetname = 'Sheet1'
                df_slim = do_enrichment_analysis(query_set, background, slim)
